window.addEventListener("load", function(){
 ...
 //   if(i.price.newUan!==undefined){
 //   return parseFloat(a.price.newUan) - parseFloat(b.price.newUan);
//}
//else{
 ...
 //     divRow.className = "row";
  ...
  for (var i of data) {
   ...
   // cell.className = "container"
    cell.innerHTML = i.name +"\<\p\>";

 //   var divCell = document.createElement("div");
    
 //   divCell.className = "cell";
  //  divCell.innerHTML = i.name+"\<\p\>";

    if(i.price.newUan!==undefined){
      cell.innerHTML +="\<\p style= color:grey;text-decoration:line-through;\>" +i.price.oldUan + "\<\p\>";
    cell.innerHTML += i.price.newUan +"\<\p\>";

  //  divCell.innerHTML += "\<\p style= color:grey;text-decoration:line-through;\>" +i.price.oldUan + "\<\p\>";
 //  divCell.innerHTML +=   i.price.newUan +"\<\p\>";
  }
    else{
      
      cell.innerHTML += i.price + "\<\p\>";
   //  divCell.innerHTML += i.price + "\<\p\>";
    }
    cell.innerHTML += i.ratingRevievs ;
   // divCell.innerHTML += i.ratingRevievs;
   ...
   //doesnt work anymore
   // cell4.append(divCell);

    if (count%perrow==0) {
      row = table.insertRow();

    //  divRow.appendChild(cell4);
